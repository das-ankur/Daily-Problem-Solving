...
public:
    ...
    */
    /*
    // Practice 4
    void helper(int idx, int target, int k, vector<int> &subset, vector<vector<int>> &res) {
        if(k==0 && target==0) {
            res.push_back(subset);
            return;
        }
        ...
        helper(idx+1,target-idx,k-1,subset,res);
        subset.pop_back();
        helper(idx+1,target,k,subset,res);
    }
    vector<vector<int>> combinationSum3(int k, int n) {
        vector<int> subset;
        vector<vector<int>> res;
